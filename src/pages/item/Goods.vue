    ...
    methods: {
      ...
      deleteGoods(oldGoods){
        this.$message.confirm("确认要删除该商品吗？")
          .then(()=> {
            // 根据品牌信息查询商品分类
            this.$http.delete("/item/spu/spuId/" + oldGoods.id)
                .then(()=>{
                  // 重新加载数据
                  this.getDataFromServer();
      ...
      // 商品上架
      upOrDownGoods(oldGoods){
        let text = "上架";
        if (oldGoods.saleable) {
          text="下架"
        }
        this.$message.confirm("确认要"+text+"该商品吗？")
          .then(()=> {
            this.$http({
              method: 'put',
              url: '/item/spu/saleable',
              data: oldGoods,
            }).then(()=>{
                // 重新加载数据
                this.getDataFromServer();
                this.$message.success("商品"+text+"成功！");
              })
              .catch(() => {
                this.$message.error("商品"+text+"失败！");
              })
            }
          )
      },
